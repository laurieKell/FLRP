...
double* myalloc1(size_t m) {
    ...
    if (m>0) {
      ...
      if (A == NULL) {
        Rprintf("ADOL-C error: myalloc1 cannot allocate %zd bytes\n",
...
double** myalloc2(size_t m, size_t n) {
    ...
    if (m>0 && n>0)  { 
      ...
      if (Adum == NULL) {
        Rprintf("ADOL-C error: myalloc2 cannot allocate %zd bytes\n",
...
double*** myalloc3(size_t m, size_t n, size_t p) { /* This function allocates 3-tensors contiguously */
    ...
    if (m>0 && n>0 && p > 0)  { 
      ...
      if (Adum == NULL) {
        Rprintf("ADOL-C error: myalloc3 cannot allocate %zd bytes\n",
...
double   **myallocI2(int n) {
    ...
    int i;
    if (Idum == NULL) {
        Rprintf("ADOL-C error: myallocI2 cannot allocate %i bytes\n",
